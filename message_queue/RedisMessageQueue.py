...
class RedisMessageQueueManager:
    ...
    def send_message(self, queue_name, message):
        ...
        logging.info("Message sent to queue '%s': %s", queue_name, message)
    
    def listen_messages(self, queue_name, callback):
        """
        ...
        """
        logging.info("Listening for messages on queue '%s'...", queue_name)
        while True:
            # BRPOP blocks until a message is available on the queue.
            _, raw_message = self.redis.brpop(queue_name)
            try:
            ...
            logging.info("Received message from queue '%s': %s", queue_name, message_obj)
